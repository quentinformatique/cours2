...
public class Evaluateur {
     ...
     * appelle construireExpEntiere() pour analyser ce texte et construire une réprésentation
    ...
    public static Object evaluer(String expression) throws ExpressionMalFormeeException {
    ...
    private static ExpressionEntiere construireExpEntiere() throws ExpressionMalFormeeException {
    ...
    /**
     * Analyser le texte mémorisé dans l'attribut texteExpression à partir de la position courante
     * de son index afin de construire une ExpressionEntiereNaire, en appelant construireExpEntiere()
    ...
    private static ExpressionEntiereNaire construireExpEntiereNaire() throws ExpressionMalFormeeException {
        ...
        try {
            ...
            // tenter de construire les opérandes puis l'expression naire à renvoyer.
                ...
                ArrayList<ExpressionEntiere> operandes = new ArrayList();
                while(c != ')') {
                    ExpressionEntiere operande = construireExpEntiere(); 
     ...
     * construireExpEntiere() pour construire les opérandes.
     * Lever ExpressionMalFormeeException si erreur de syntaxe.
     */

    private static ExpressionEntiereBinaire construireExpEntiereBinaire() throws ExpressionMalFormeeException {
           char c;
        int indexDepart = texteExpression.getIndex();
        try {
            ...
            // construire les opérandes gauche et droite puis l'expression binaire à renvoyer.
                ...
                if (texteExpression.caractereSuivant()!=' ') 
                    throw new ExpressionMalFormeeException();
                ExpressionEntiere gauche = construireExpEntiere(); 
                texteExpression.caractereSuivant(); 
                ExpressionEntiere droite = construireExpEntiere();

            // 6. Si l'opérande droite n'est pas suivi d'une parenthèse fermante, l'expression est 
            // mal formée. Sinon, contruire et renvoyer l'expression entière binaire.
                if(texteExpression.caractereNonEspaceSuivant()!=')') 
                    throw new ExpressionMalFormeeException();
                else return new ExpressionEntiereBinaire(operateur,gauche,droite);
            }
        }
        catch(FinTexteException e) {
            throw new ExpressionMalFormeeException();
    ...
    /**
     * Analyser le texte mémorisé dans l'attribut texteExpression à partir de la position courante 
     * de son index afin de construire soit une Comparaison, soit une ExpressionLogiqueNaire, soit
     * une Negation, en appelant construireComparaison(), constrruireExpLogiqueNaire() et 
     * contsruireNegation(). Lever ExpressionMalFormeeException si erreur de syntaxe.
     */
    
    private static ExpressionLogique construireExpLogique() throws ExpressionMalFormeeException {
        // A COMPLETER : code similaire à la construction d'une expression entière
        return null; // Bouchon
    }

    /**
     * Analyser le texte mémorisé dans l'attribut texteExpression à partir de la position courante
     * de son index afin de construire une Comparaison, en appelant construireExpEntiere() pour 
     * construire les opérandes. Lever NumberFormatException si erreur de syntaxe.
     */
    
    private static Comparaison construireComparaison() throws ExpressionMalFormeeException {
        // A COMPLETER : code similaire à la construction d'une expression entière binaire
        return null; // Bouchon
    }

    /**
     * Analyser le texte mémorisé dans l'attribut texteExpression à partir de la position courante 
     * de son index afin de construire une ExpressionLogiqueNaire, en appelant construireExpLogique()
     * pour construire les opérandes. Lever ExpressionMalFormeeException si erreur de syntaxe.
     */
    
    private static ExpressionLogiqueNaire construireExpLogiqueNaire() throws ExpressionMalFormeeException {
        // A COMPLETER : code similaire à la construction d'une expression entière naire
        return null; // Bouchon
    }

    /**
     * Analyser le texte mémorisé dans l'attribut texteExpression à partir de la position courante de
     * son index afin de construire une Negation, en appelant construireExpLogique() pour construire
     * l'opérande. Lever ExpressionMalFormeeException si erreur de syntaxe.
     */
    
    private static Negation construireNegation() throws ExpressionMalFormeeException {
        // A COMPLETER : code similaire à la construction d'une expression entière binaire
        // excepté le fait qu'il faut construire qu'un seul opérande.
        return null; // Bouchon
    }

    /**
     * Analyser le texte mémorisé dans l'attribut texteExpression à partir de la position courante de
     * son index afin de construire soit une ExpressionEntiere, soit une ExpressionLogique, en 
     * appelant  construireExpEntiere() et contsruireExpLogique().
     * Lever ExpressionMalFormeeException si erreur de syntaxe.
     */
